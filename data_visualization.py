...
def create_1st_stage_charts(stats_df):
    ...
    set_labels_font_size()
    sorted_stats = stats_df.sort_values('contigs', ascending=False).reset_index(drop=True)
    contigs = plt.bar(sorted_stats.index.values, sorted_stats['contigs'], width)
    singletons = plt.bar(sorted_stats.index.values + width, sorted_stats['singletons'], width)
    plt.xlabel("strains #")
    plt.ylabel("contigs #")
    plt.title("strains to contigs VS singletons bar chart")
    plt.legend((contigs[0], singletons[0]), ("Contigs #", "Singletons #"))
