...
def perform_alignment_and_pruning(worker_id, job_queue, configurer, log_queue):
    ...
    while True:
        ...
        mafft_return_code = run(mafft_args, shell=True, stdout=cluster_alignment_file, sdterr=alignment_stderr).returncode
        ...
        gblocks_return_code = run(gblocks_args, shell=True, stdout=alignment_stdout, sdterr=alignment_stderr).returncode
        logger.info(
